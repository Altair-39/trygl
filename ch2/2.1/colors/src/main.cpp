...
void renderScene(GLFWwindow *window, unsigned int cubeVAO,
                 unsigned int lightVAO, Shader &lightingShader,
                 Shader &lightCubeShader);
void cleanup(unsigned int cubeVAO, unsigned int lightVAO, unsigned int VBO,
...
int main() {
  ...
  // Initialize shaders
  lightingShader = new Shader("colors.vs", "colors.fs");
  lightCubeShader = new Shader("light_cube.vs", "light_cube.fs");

  // Setup vertex data and buffers
  unsigned int cubeVAO = setupCubeVAO();
  unsigned int lightVAO = setupLightVAO();

  // Load textures (optional - for textured cubes)
  // texture1 = createTexture("texture.jpg");
  // texture2 = createTexture("awesomeface.png", true);

  // Main render loop
  renderScene(window, cubeVAO, lightVAO, *lightingShader, *lightCubeShader);

  // Cleanup
  cleanup(cubeVAO, lightVAO, 0, texture1, texture2);
...
unsigned int setupLightVAO() {
  // Use the same vertex data as the cube
  float vertices[] = {
      -0.5f, -0.5f, -0.5f, 0.5f,  -0.5f, -0.5f, 0.5f,  0.5f,  -0.5f,
      0.5f,  0.5f,  -0.5f, -0.5f, 0.5f,  -0.5f, -0.5f, -0.5f, -0.5f,
