...
int main() {
  ...
  // SET THE TEXTURE UNIT - THIS IS THE KEY FIX
  ourShader.use();
  ourShader.setInt(
      "ourTexture",
      0); // Make sure "ourTexture" matches your fragment shader sampler name
  ...
  unsigned char *data =
      stbi_load("texture.jpg", &width, &height, &nrChannels, 0);
  if (data) {
