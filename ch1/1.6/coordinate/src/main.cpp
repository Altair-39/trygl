...
void renderScene(GLFWwindow *window, unsigned int VAO, unsigned int texture1,
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Set up view and projection matrices (these are the same for all cubes)
    glm::mat4 view = glm::mat4(1.0f);
    view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));
    glm::mat4 projection =
        glm::perspective(glm::radians(45.0f), 800.0f / 600.0f, 0.1f, 100.0f);

    // Get uniform locations after shader is active
    ...
    for (unsigned int i = 0; i < 10; i++) {
      ...
      float angle =
          20.0f * i + (float)glfwGetTime() * 50.0f; // Add rotation over time
      model =
          glm::rotate(model, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));

      // Set model uniform for this cube
